...
def preprocess_image(img):
    ...
    # 去噪
    img = cv2.medianBlur(img, 5) # 要是奇數 能大幅影響辨識出來的結果
    return img
...
if len(most_common_plate) != 7:
    ...
    # 从文件中读取车牌号码和入场时间
    entry_time = None
    if os.path.exists(entry_log_path):
        with open(entry_log_path, 'r') as log_file:
            lines = log_file.readlines()

        with open(entry_log_path, 'w') as log_file:
            for line in lines:
                plate, time = line.strip().split(',')
                if plate == most_common_plate:
                    entry_time = time
                else:
                    log_file.write(line)

    # 如果找到了入场时间，打印入场时间和当前时间
    if entry_time:
        print(f"\nsuccess recognize: {most_common_plate}")
        print(f"Entry Time: {entry_time}")
        print(f"Exit Time: {current_time}")
    else:
        print(f"ERROR: No entry record found for plate: {most_common_plate}")